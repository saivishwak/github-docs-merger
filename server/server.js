  ...
  if (response.status === 200) {
    ...
    for (const item of content) {
      ...
      const shouldExclude = excludeKeywords.some((keyword) => {
        return itemNameLowerCase.includes(keyword) || itemPathLowerCase.includes(keyword);
      });
      ...
      if (
        ...
        const subMarkdownBuffers = await fetchMarkdownFiles(username, repoName, subDirectoryPath);
